...
function TampilLaporanTahunTahunSayaUser(){
  ...
  const piePekerjaan = [
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        Axios.get(`https://backend-fix.glitch.me/laporan/getlaporantahunan/${id_user}/${dataTahun}`, {
            headers: {
                "Authorization": `Bearer ${token}`,
            }
        }).then((response) => {
    ...
    return (
        <div >
            <SubTitleComponent title="Laporan" subtitle="Laporan Tahunan Saya"/>
            ...
            <MDBBtn 
                ...
                onClick={(e) => {
                    exportToCSV(data, `laporan_tahunan_${dataTahun}.csv`)
                }}>Export Laporan
            </MDBBtn>
        </div>
    )           
...
export default TampilLaporanTahunTahunSayaUser;